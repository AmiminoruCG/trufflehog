...
func TestPerplexity_Pattern(t *testing.T) {
	...
	canonicalKey := fakeKey("1234567890abcdef")
	longKey := fakeKey("7b8c9d0e1f2a3b4c5d6e7f8091a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b")
	firstKey := fakeKey(strings.Repeat("a", 32))
	secondKey := fakeKey(strings.Repeat("b", 32))
	...
	tests := []struct {
	...
	}{
		{
			...
			input: "PERPLEXITY_API_KEY = '" + canonicalKey + "'",
			want:  []string{canonicalKey},
		},
		{
			name:  "long API key",
			input: "PPLX_API_KEY=" + longKey,
			want:  []string{longKey},
		},
		{
			name: "finds all unique matches",
			input: `perplexity_key = "` + firstKey + `"
PERPLEXITY_API_KEY = "` + secondKey + `"`,
			want: []string{
				firstKey,
				secondKey,
			},
		...
		{
			name:  "too short",
			input: "PERPLEXITY_API_KEY = " + fakeKey("short"),
			want:  nil,
		},
		{
			name:  "wrong provider",
			input: "perplexity_key = " + fakeOpenRouterKey("7b8c9d0e1f2a3b4c5d6e7f8091a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b"),
			want:  nil,
		},
	}
...
func TestPerplexity_Verify(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "verified",
			client:       common.ConstantResponseHttpClient(200, `{"requests":[]}`),
			wantVerified: true,
		},
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			results, err := d.FromData(context.Background(), true, []byte("PERPLEXITY_API_KEY="+verifyKey))
			if err != nil {
...
func fakeOpenRouterKey(suffix string) string {
